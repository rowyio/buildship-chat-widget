...
In case of a stream response, the widget won't expect a JSON object as described above, but will instead expect a stream of chunks that eventually add up to the response message. The widget will aggregate these chunks as they're received and display and update the message in real time, finally ending with the full response. Optionally, to set the `threadId` through the response, the endpoint may respond with the `message` and the `threadId` in the following format: `message` + `\x1f` + `threadId`, where `\x1f` is the unit separator character. For example:
...
The widget will then extract the thread ID from the stream and set it as the `threadId` for the conversation.
